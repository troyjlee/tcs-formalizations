...
/-!
# KKO21 Lemma 7.10: a boundary atom is happy with probability `≥ (1−a)² + a² − …`

For a polygon cut `Ŝ` with polygon partition `Â, B̂, Ĉ` and its leftmost atom
`S = a₁` (or rightmost, `a_{m−1}`), with `S`'s own polygon partition `A, B, C`,

`P[S happy | R_Ŝ] ≥ (1 − x(A→))² + x(A→)² − 2ε_M − c·ε_η`,

`A→ = A ∩ E(Ŝ)`.  Under the max-flow selection `v` of `Ŝ` exactly one edge of
the boundary group `G = δ(S) ∩ δ(Ŝ)` (`= Â` for the leftmost atom) is
present, so `S` is happy as soon as **either** the present boundary edge is in
`A` and the inside picks exactly one edge of `B` and none of `A`, `C`, **or**
symmetrically.  The two events are disjoint, each is an outside-determined
event times an inside-determined one, and Corollary 5.9's cross form
(`weightMass_selected_cross`) factors them: the outside factor is the selected
marginal of `A↑` (`≥ x(A↑) − ε_M − ε_η/2`, from `tvA` and the face/avoid
transfer), the inside factor is a mass under the polygon law `ν`, bounded
below through the **defect lemma** `weightMass_singleton_ge`: since `Ŝ`
induces a tree, `δ→(S)` is met at least once, so
`P_ν[A→ = 0, B→ = 1, C→ = 0] ≥ E_ν[B→] − 2(E_ν[δ→] − 1)`, and `E_ν[δ→] ≤
x(δ→) + 3.5ε_η ≤ 1 + 5.5ε_η`.  The arithmetic closes at `2ε_M + 40ε_η`,
`ε_M = 0.00025`.

⚠️ The paper's `17ε_η` used `x(C) ≤ ε_η`; the near-cycle interface exposes
`x(C) ≤ 3ε_η` (`sum_partC_le`), so the honest coefficient is larger.  Lemma
7.9 has room for it.
-/
...
/-- On `X + Y + Z ≥ 1`, `Y − 1{X = 0, Y = 1, Z = 0} ≤ 2(X + Y + Z − 1)`. -/
theorem defect_pointwise {X Y Z : ℕ} (h : 1 ≤ X + Y + Z) :
    (Y : ℝ) - (if X = 0 ∧ Y = 1 ∧ Z = 0 then 1 else 0) ≤ 2 * ((X : ℝ) + Y + Z - 1) := by
  have hX0 : (0:ℝ) ≤ X := by positivity
  have hZ0 : (0:ℝ) ≤ Z := by positivity
  have hsum : (1:ℝ) ≤ X + Y + Z := by exact_mod_cast h
  split_ifs with hc
  · obtain ⟨hX, hY, hZ⟩ := hc
    subst hX hY hZ
    norm_num
  · by_cases hY : Y = 0
    · subst hY; push_cast at hsum ⊢; linarith
    · by_cases hY1 : Y = 1
      · subst hY1
        have hXZ : 1 ≤ X + Z := by
          rcases Nat.eq_zero_or_pos X with hX | hX
          · rcases Nat.eq_zero_or_pos Z with hZ | hZ
            · exact absurd ⟨hX, rfl, hZ⟩ hc
            · omega
          · omega
        have : (1:ℝ) ≤ X + Z := by exact_mod_cast hXZ
        push_cast at hsum ⊢; linarith
      · have hY2 : 2 ≤ Y := by omega
        have : (2:ℝ) ≤ Y := by exact_mod_cast hY2
        push_cast at hsum ⊢; linarith

/-- **The defect lemma**: when every supported set meets `A ∪ B ∪ C`, the mass
of "exactly one edge, in `B`" is at least `E[B] − 2(E[A] + E[B] + E[C] − 1)`. -/
theorem weightMass_singleton_ge {w : Finset ι → ℝ} (hnn : WeightNonneg w)
    (htot : totalMass w = 1) {A B C : Finset ι}
    (hone : ∀ T, w T ≠ 0 → 1 ≤ (T ∩ A).card + (T ∩ B).card + (T ∩ C).card) :
    expCard w B - 2 * (expCard w A + expCard w B + expCard w C - 1)
      ≤ weightMass w (fun T => (T ∩ A).card = 0 ∧ (T ∩ B).card = 1 ∧ (T ∩ C).card = 0) := by
  rw [← htot]
  unfold expCard totalMass weightMass
  dsimp only
  have hpt : ∀ T, w T * ((T ∩ B).card : ℝ)
      - 2 * (w T * ((T ∩ A).card : ℝ) + w T * ((T ∩ B).card : ℝ) + w T * ((T ∩ C).card : ℝ)
        - w T)
      ≤ (if (T ∩ A).card = 0 ∧ (T ∩ B).card = 1 ∧ (T ∩ C).card = 0 then w T else 0) := by
    intro T
    by_cases hT : w T = 0
    · rw [hT]; split_ifs <;> simp
    · have hw := hnn T
      have h := defect_pointwise (hone T hT)
      ...
      · rw [if_neg hc] at h'
        nlinarith
  calc ∑ T, w T * ((T ∩ B).card : ℝ)
        - 2 * (∑ T, w T * ((T ∩ A).card : ℝ) + ∑ T, w T * ((T ∩ B).card : ℝ)
          + ∑ T, w T * ((T ∩ C).card : ℝ) - ∑ T, w T)
      = ∑ T, (w T * ((T ∩ B).card : ℝ)
          - 2 * (w T * ((T ∩ A).card : ℝ) + w T * ((T ∩ B).card : ℝ)
            + w T * ((T ∩ C).card : ℝ) - w T)) := by
        rw [Finset.sum_sub_distrib, ← Finset.mul_sum, Finset.sum_sub_distrib,
          Finset.sum_add_distrib, Finset.sum_add_distrib]
    _ ≤ ∑ T, (if (T ∩ A).card = 0 ∧ (T ∩ B).card = 1 ∧ (T ∩ C).card = 0 then w T else 0) :=
        Finset.sum_le_sum fun T _ => hpt T
    _ = _ := Finset.sum_congr rfl fun T _ => by
        rw [Subsingleton.elim (instDecidableAnd) (Classical.propDecidable _)]

end Generic

/-! ### The arithmetic -/

/-- One product term: `X ≥ b − 12ε_η`, `Y ≥ a' − ε_M − ε_η/2 ≥ b − 3ε_η − ε_M − ε_η/2`,
both nonnegative. -/
theorem lemma_7_10_product {εη b a' X Y : ℝ} (hεη : 0 ≤ εη) (hεηcap : εη ≤ 0.00000004)
    (hb : 0 ≤ b) (ha'b : b - 3 * εη ≤ a')
    (hX : b - 26 * εη ≤ X) (hX0 : 0 ≤ X) (hY : a' - (0.00025 + εη / 2) ≤ Y) (hY0 : 0 ≤ Y) :
    b ^ 2 - b * (0.00025 + 29.5 * εη) - εη ≤ X * Y := by
  have hXY : 0 ≤ X * Y := mul_nonneg hX0 hY0
  have hk : (0:ℝ) ≤ 0.00025 + 29.5 * εη := by linarith
  ...
  · have h1 : b * b ≤ b * (26 * εη) := mul_le_mul_of_nonneg_left hb1.le hb
    ...
    have h4 : 0 ≤ b * (0.00025 + 29.5 * εη) := mul_nonneg hb hk
    nlinarith
  · push Not at hb1
    by_cases hb2 : b < 3 * εη + (0.00025 + εη / 2)
    · have h1 : b * (b - (0.00025 + 29.5 * εη)) ≤ 0 :=
        mul_nonpos_of_nonneg_of_nonpos hb (by linarith)
      nlinarith
    · push Not at hb2
      have hY' : b - 3 * εη - (0.00025 + εη / 2) ≤ Y := by linarith
      have h := mul_le_mul hX hY' (by linarith) hX0
      have hid : (b - 26 * εη) * (b - 3 * εη - (0.00025 + εη / 2))
          = b ^ 2 - b * (0.00025 + 29.5 * εη) + 26 * εη * (3 * εη + (0.00025 + εη / 2)) := by
        ring
      have h0 : 0 ≤ 26 * εη * (3 * εη + (0.00025 + εη / 2)) := by positivity
      nlinarith

/-- **Lemma 7.10's arithmetic.**  `a, a'` are `x(A→), x(A↑)`, `b, b'` are `x(B→),
x(B↑)`, `c = x(C→)`; `X₁, X₂` the two inside masses, `Y₁, Y₂` the two normalized
outside marginals. -/
theorem lemma_7_10_arith {εη a a' b b' c X1 Y1 X2 Y2 : ℝ}
    (hεη : 0 ≤ εη) (hεηcap : εη ≤ 0.00000004)
    (ha : 0 ≤ a) (hb : 0 ≤ b) (hc : 0 ≤ c)
    (hA : 1 - εη ≤ a + a') (hB : 1 - εη ≤ b + b')
    (harrow : a + b + c ≤ 1 + 2 * εη) (harrow' : 1 - 2 * εη ≤ a + b + c) (hc3 : c ≤ 3 * εη)
    (hX1 : b - 26 * εη ≤ X1) (hX10 : 0 ≤ X1) (hY1 : a' - (0.00025 + εη / 2) ≤ Y1) (hY10 : 0 ≤ Y1)
    (hX2 : a - 26 * εη ≤ X2) (hX20 : 0 ≤ X2) (hY2 : b' - (0.00025 + εη / 2) ≤ Y2) (hY20 : 0 ≤ Y2) :
    (1 - a) ^ 2 + a ^ 2 - 0.0005 - 40 * εη ≤ X1 * Y1 + X2 * Y2 := by
  have ha'b : b - 3 * εη ≤ a' := by linarith
  have hb'a : a - 3 * εη ≤ b' := by linarith
  have hP1 := lemma_7_10_product hεη hεηcap hb ha'b hX1 hX10 hY1 hY10
  have hP2 := lemma_7_10_product hεη hεηcap ha hb'a hX2 hX20 hY2 hY20
  have hsq : (1 - a) ^ 2 ≤ b ^ 2 + 10 * εη := by
    ...
    · have hb5 : 1 - a - 5 * εη ≤ b := by linarith
      ...
      nlinarith
  have hk : (0:ℝ) ≤ 0.00025 + 29.5 * εη := by linarith
  have hab : (a + b) * (0.00025 + 29.5 * εη) ≤ (1 + 2 * εη) * (0.00025 + 29.5 * εη) :=
    mul_le_mul_of_nonneg_right (by linarith) hk
  have hsmall : (1 + 2 * εη) * (0.00025 + 29.5 * εη) ≤ 0.00025 + 30 * εη := by nlinarith
  nlinarith

/-! ### The core, at an abstract boundary group -/

/-- The support of the polygon law induces a tree on the cut. -/
theorem polygonLaw_inducesTreeOn {μ : TreeDist n x} {Ŝ : Finset (Fin n)}
    {C : Finset (Sym2 (Fin n))} (hŜne : Ŝ.Nonempty) {T : Finset (Sym2 (Fin n))}
    (hT : polygonLaw μ Ŝ C T ≠ 0) : InducesTreeOn Ŝ T := by
  have h1 : treeFace μ.prob Ŝ T ≠ 0 := (avoidDist_ne_zero_imp hT).1
  obtain ⟨hprob, hcost⟩ := faceDist_ne_zero_imp h1
  have hTst := μ.support_spanningTree T hprob
  rw [setCost_indicatorCost] at hcost
  have h1S : 1 ≤ Ŝ.card := Finset.card_pos.mpr hŜne
  have hcard : (internalEdges Ŝ ∩ T).card + 1 = Ŝ.card := by
    rw [Finset.inter_comm]
    omega
  exact (inducesTreeOn_iff_card hTst Ŝ).mpr hcard

set_option maxHeartbeats 4000000 in
-- the two factorized events, their outside marginals, their inside masses, and the arithmetic
/-- **Lemma 7.10's core**, for a child `S` of `Ŝ` whose boundary group `G = δ(S) ∩
δ(Ŝ)` is met exactly once on the selection, with a total-variation bound on `G`. -/
theorem lemma_7_10_core (hx : IsRestrictedLP e₀ x) {μ : TreeDist n x} (hμ : IsMaxEntropyLimit μ)
    ...
    ((1 - ∑ g ∈ N.partA ∩ internalEdges Ŝ, x g) ^ 2
        + (∑ g ∈ N.partA ∩ internalEdges Ŝ, x g) ^ 2 - 0.0005 - 40 * εη) * totalMass v
  ...
  have hP1in : InsideDetermined Ŝ P1 :=
    (insideDetermined_inter (fun e he => (mem_internalEdges.mp (hAinF he)).2) (fun X => X.card = 0)).and
      ((insideDetermined_inter (fun e he => (mem_internalEdges.mp (hBinF he)).2) (fun X => X.card = 1)).and
        (insideDetermined_inter (fun e he => (mem_internalEdges.mp (hCinF he)).2) (fun X => X.card = 0)))
  have hP2in : InsideDetermined Ŝ P2 :=
    (insideDetermined_inter (fun e he => (mem_internalEdges.mp (hBinF he)).2) (fun X => X.card = 0)).and
      ((insideDetermined_inter (fun e he => (mem_internalEdges.mp (hAinF he)).2) (fun X => X.card = 1)).and
        (insideDetermined_inter (fun e he => (mem_internalEdges.mp (hCinF he)).2) (fun X => X.card = 0)))
  ...
  have houtside : ∀ D ⊆ G, M * ((∑ e ∈ D, x e) - (0.00025 + εη / 2)) ≤ expCard v D := by
    intro D hDG
    ...
    rw [← hν'] at hlo
    rw [hxD] at hlo
    have := mul_le_mul_of_nonneg_left (show (∑ e ∈ D, x e) - εη / 2 ≤ expCard ν D by linarith) hMpos.le
    nlinarith
  -- the inside means
  ...
  have hQ1eq : weightMass v Q1 = expCard v Aup := (expCard_eq_weightMass_one (hone_up Aup hAupG)).symm
  have hQ2eq : weightMass v Q2 = expCard v Bup := (expCard_eq_weightMass_one (hone_up Bup hBupG)).symm
  ...
  calc ((1 - ∑ g ∈ N.partA ∩ internalEdges Ŝ, x g) ^ 2
        + (∑ g ∈ N.partA ∩ internalEdges Ŝ, x g) ^ 2 - 0.0005 - 40 * εη) * M
...
/-- **KKO21 Lemma 7.10**, at the bottom thinning of `Ŝ`: for the leftmost atom
`S = a₁`, `((1 − a)² + a² − 0.0005 − 40ε_η) · p ≤ W_{R_Ŝ}[S happy]`, `a = x(A→)`. -/
theorem lemma_7_10_left (hx : IsRestrictedLP e₀ x) {μ : TreeDist n x} (hμ : IsMaxEntropyLimit μ)
    {H : Hierarchy x e₀ εη} (hεη : 0 ≤ εη) (hεηcap : εη ≤ 0.00000004)
    ...
    ((1 - ∑ g ∈ N.partA ∩ internalEdges Ŝ, x g) ^ 2
        + (∑ g ∈ N.partA ∩ internalEdges Ŝ, x g) ^ 2 - 0.0005 - 40 * εη) * p
  ...
  have hcore := lemma_7_10_core hx hμ H hεη hεηcap hŜ hpres W.polygon hSŜ hN hGup
...
theorem lemma_7_10_right (hx : IsRestrictedLP e₀ x) {μ : TreeDist n x} (hμ : IsMaxEntropyLimit μ)
    {H : Hierarchy x e₀ εη} (hεη : 0 ≤ εη) (hεηcap : εη ≤ 0.00000004)
    {Ŝ : Finset (Fin n)} (hŜ : Ŝ ∈ H.cuts) {p : ℝ} (hp : 0 ≤ p)
    {Ξ : BottomThinning H μ Ŝ p} (hG : BottomGuarantees H μ Ŝ p Ξ)
