...
/-!
# Capacity-strengthened Lemma 5.22

The `_capacity` export gives `0.026ε₂²`, by instantiating
`lemma_5_22_indexed_capacity` at the identity fiber tree model.
...
/-- **KKO21 Lemma 5.22.** -/
theorem lemma_5_22_capacity {w : Finset (Sym2 (Fin n)) → ℝ} {k : ℕ}
    ...
    (hxE : 1 / 2 + ε₂ ≤ expCard w (betweenEdges u v))
  ...
  have hcount : (FiberTreeModel.id n).TwoAtomOneHotData w u v :=
    FiberTreeModel.TwoAtomOneHotData.ofSupport hsupp hune hvne huv
  have h := lemma_5_22_indexed_capacity (FiberTreeModel.id n) hst hr hnn htot hune hvne huv hcount
...
theorem lemma_5_22 {w : Finset (Sym2 (Fin n)) → ℝ} {k : ℕ}
    (hst : IsRealStable (genPoly w)) (hr : FixedRankWeight (k + 1) w)
    (hnn : WeightNonneg w) (htot : totalMass w = 1)
    (htree : ∀ T, w T ≠ 0 → IsSpanningTree n T)
    {u v : Finset (Fin n)} (hune : u.Nonempty) (hvne : v.Nonempty) (huv : Disjoint u v)
    ...
    0.005 * ε₂ ^ 2 ≤ weightMass w (fun T =>
      (T ∩ A).card = 1 ∧ (T ∩ B).card = 1 ∧ (T ∩ C).card = 0
        ∧ (T ∩ cutEdges v).card = 2 ∧ InducesTree u T ∧ InducesTree v T) := by
  have h := lemma_5_22_capacity hst hr hnn htot htree hune hvne huv
