...
/-!
# Capacity-strengthened Lemma 5.21 on pieces

The `_capacity` export gives `0.04ε₂²`, by instantiating
`lemma_5_21_indexed_capacity` at the refined fiber tree model.
...
/-- **Lemma 5.21 for the lifted law, with piece sides.** -/
theorem lemma_5_21_liftProb_capacity {e₀ : RootEdge n} (hx : IsRestrictedLP e₀ x) (μ : TreeDist n x)
    ...
    R.IsTwoOneOneGoodOn μ (0.04 * ε₂ ^ 2) u v A B C := by
  have P := R.refinedPairData hμ (H.pairData hx μ hμ hu hv huv)
  have h := lemma_5_21_indexed_capacity R.model P.stable P.rank (R.liftProb_weightNonneg μ)
    (R.liftProb_totalMass μ) P.une P.vne P.disj P.count (A := A) (B := B) (C := C)
...
theorem lemma_5_21_liftProb {e₀ : RootEdge n} (hx : IsRestrictedLP e₀ x) (μ : TreeDist n x)
  ...
  have h := lemma_5_21_liftProb_capacity hx μ hμ H R hu hv huv
